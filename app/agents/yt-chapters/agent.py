...
@lru_cache(maxsize=1)
def _build_yt_chapters_agent():
    return create_deep_agent(
        model="openai:gpt-5.4",
        memory=["./agents/yt-chapters/AGENTS.md"],
        backend=FilesystemBackend(root_dir=".", virtual_mode=True),
    )


def create_chapters(transcript: str, content: str, language: str) -> str:
    agent = _build_yt_chapters_agent()
    result = agent.invoke(
        {
            "messages": [
                {
        ...
        config={"configurable": {"thread_id": f"yt-chapters-{uuid4()}"}},
    )
    ...
    chapters = extract_text_content(message_content)
    if not chapters:
        raise ValueError("Deep Agent returned empty chapters.")
