...
@lru_cache(maxsize=1)
def _build_yt_title_agent():
    return create_deep_agent(
        model="openai:gpt-5.4",
        memory=["./agents/yt_title/AGENTS.md"],
        backend=FilesystemBackend(root_dir=".", virtual_mode=True),
    )


def create_titles(content: str, language: str) -> str:
    agent = _build_yt_title_agent()
    result = agent.invoke(
        {
            "messages": [
                {
        ...
        config={"configurable": {"thread_id": f"yt-title-{uuid4()}"}},
    )
    ...
    titles = extract_text_content(message_content)
    if not titles:
        raise ValueError("Deep Agent returned empty titles.")
